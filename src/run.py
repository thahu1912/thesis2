...
from lightning.bayesian_model import BayesianModel
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        default="../configs/cub200/bayesian.yaml",
        type=str,
        help="config file",
    )
    ...
    # Set MC Dropout flag only for triplet loss (optionally restrict to densenet161)
    if hasattr(config, "loss") and config.loss == "triplet":
        config.use_mc_dropout = True
    else:
        config.use_mc_dropout = False
    print(config)
